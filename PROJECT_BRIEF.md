...
- **Bug remonté (usage réel mobile, 04/09/2026, non investigué)** : en tapant du texte dans un champ de recherche (repro donné par l'utilisateur : taper "Huile" dans le flux d'ajout d'aliment — capture d'écran à l'appui), le texte tapé n'est **pas visible à l'écran** une fois le clavier virtuel ouvert — la liste de résultats de recherche s'affiche (donc le champ réagit bien à la saisie), mais le champ de saisie lui-même est caché, scrollé au-dessus de la zone visible. **Signalé comme non isolé à ce seul endroit** — au moins un autre champ concerné, non précisé par l'utilisateur. Possiblement apparenté au mécanisme de remontage du sheet au-dessus du clavier (`SHEETS_TO_LIFT`, voir session du 04/09/2026 et son clamp `sheetRect.top - 8px`) sans être forcément le même bug ni couvert par ce clamp — à vérifier, pas supposé. **À investiguer avant de corriger** (voir `boucles_de_verification.md`) : lister précisément quel(s) champ(s)/quelle(s) modale(s) sont touchés (pas seulement la recherche d'aliment), reproduire sur un appareil réel si possible (le clavier virtuel mobile ne se simule pas fidèlement depuis Claude in Chrome desktop), avant de proposer un correctif — ne pas réutiliser à l'aveugle le fix du 04/09 qui visait un symptôme voisin mais pas forcément la même cause.
