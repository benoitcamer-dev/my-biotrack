...
- ~~Bug remonté (usage réel mobile, 04/09/2026, non investigué)~~ **résolu le 04/09/2026** : repro confirmé via vidéo utilisateur (frames extraites à la main, ffmpeg local) — dans `#modal-add` (ajout d'aliment), en tapant "Huile" clavier ouvert, le champ `#in-desc` et la liste `#search-results` étaient scrollés au-dessus de la zone visible, cachés sous la barre de statut.
  ...
  - Le mécanisme JS `SHEETS_TO_LIFT`/`translateY` (`app.js:6895-6961`, fix du 04/09 précédent) est **conservé tel quel** : il traite un souci distinct (ramener le bouton Valider au-dessus du clavier sur un formulaire long) et n'était pas la cause de ce bug-ci — conformément à la consigne de ne pas le réutiliser à l'aveugle.
  - **Non vérifié sur appareil mobile réel** (clavier virtuel non simulable fidèlement depuis les outils desktop) — vérifié uniquement par simulation JS de la réduction de hauteur ; à confirmer en usage réel avant de considérer le point définitivement clos.
